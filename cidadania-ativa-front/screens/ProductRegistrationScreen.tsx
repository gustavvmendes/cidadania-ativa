...
const ProductRegistrationScreen = ({ navigation }: Props) => {
  ...
  const [vigenciaOferta, setVigenciaOferta] = useState('');
  ...
  const handleRegisterPress = async () => {
    if (!nomeProduto || !preco || !descricao) {
      Alert.alert('Erro de Cadastro', 'Por favor, preencha todos os campos obrigatórios (Nome, Preço e Descrição).');
      return;
    }
    ...
    if (isNaN(precoNumerico) || precoNumerico <= 0) {
      Alert.alert('Erro de Cadastro', 'Por favor, insira um preço válido maior que zero.');
      return;
    }
    ...
    try {
      ...
      formData.append('tipo_anuncio', 'produto');

      // Adicionar data de validade se fornecida
      if (vigenciaOferta) {
        formData.append('data_validade', vigenciaOferta);
      }
      ...
      if (selectedImageUri) {
        ...
        formData.append('imagem', {
          ...
          name: `produto-${Date.now()}.${fileType}`,
          type: `image/${fileType}`,
        } as any);
      }
      ...
      if (response.ok && data.success) {
        ...
        Alert.alert('Erro de Cadastro', data.message || 'Ocorreu um erro ao cadastrar o produto. Tente novamente.');
      }
    } catch (error: any) {
      console.error('Erro ao tentar cadastrar produto:', error);
      Alert.alert(
        'Erro de Conexão',
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        <TextInput
          style={styles.input}
          placeholderTextColor={theme.colors.placeholder}
          placeholder="Vigência da Oferta (AAAA-MM-DD) - Opcional"
          onChangeText={setVigenciaOferta}
          value={vigenciaOferta}
          editable={!loading}
        />
        ...
        >
          <Text style={styles.imagePickerText}>
            {selectedImageUri ? 'Imagem selecionada ✔️' : '📷 Selecione uma imagem do produto (Opcional)'}
          </Text>
        </TouchableOpacity>
