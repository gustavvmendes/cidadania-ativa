...
interface ProductDetailsRouteParams {
  productId: string;
  name: string;
  price: string; // Vem como string, será convertido
...
const ProductDetailsScreen: React.FC<Props> = ({ route, navigation }) => {
  ...
  // Função para validar e converter o ID do produto
  ...
  const fetchComments = async () => {
    ...
    try {
      // Validar o ID do produto antes de fazer a requisição
      const validProductId = validateProductId(product.productId);

      if (!validProductId) {
        setErrorComments('ID do produto inválido.');
      ...
      const response = await fetch(`${API_BASE_URL}/anuncios/${validProductId}/comentarios`, {
        method: 'GET',
        headers: headers,
      });
  ...
  const handleBuyPress = () => {
    ...
    Alert.alert('Funcionalidade', 'A funcionalidade de compra ainda não foi implementada.');
  };
  ...
  const handleImageError = () => {
    ...
    product.image = require('../assets/images/placeholder-product.png');
  };
  ...
  // Converter o preço para número antes de usar toFixed
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        <View style={styles.detailsContainer}>
          <View style={styles.infoSection}>
            ...
            {product.tipo_anuncio === 'produto' && (
              <Text style={styles.productPrice}>{formattedPrice}</Text>
            )}
            {product.tipo_anuncio === 'evento' && product.data_evento && product.local_evento && (
              <View>
      ...
      <View style={styles.buyButtonWrapper}>
        <AppButton
          title={product.tipo_anuncio === 'produto' ? 'Comprar! ➡' : 'Participar! ➡'}
